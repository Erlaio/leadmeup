...
def currency_parse(date_month: int, date_year: int) -> float:
    ...
    request = requests.get(
        'http://www.cbr.ru/scripts/XML_daily.asp?date_req=01/{}/{}'.format(
            date_check(date_month), date_year))
    data = BeautifulSoup(request.text, 'lxml')

    return float(data.find('name', string='Доллар США').find_next('value').text.replace(',', '.'))


current_date = datetime.datetime.now()
current_date_month = int(input('\nТекущий месяц-год: {}-{}\n'
